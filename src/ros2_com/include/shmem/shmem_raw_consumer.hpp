...
*/
#ifndef SHMEM_RAW_CONSUMER_HPP
#define SHMEM_RAW_CONSUMER_HPP
...
namespace zbot::shmem
{
	...
	*/
	...
	public:
		...
		*/
		...
		AsyncWaiter(SharedObjectSharedPtr& t_sharedObj, ConsumerWeakPtr& t_consumerRef, OnRecieveCallback&& t_onRecieve)
	...
	private:
		...
		*/
		SharedObjectSharedPtr m_sharedObj;
		/*!
		 * @brief Weak pointer to the consumer reference
		*/
		ConsumerWeakPtr m_consumerRef;
	};
	...
	*/
	...
	class ShmemRawConsumer : public ShmemConsumerBase<SharedRawObject<T, StorageType>, StorageType>
	{
	public:
		using AsyncW = AsyncWaiter<SharedRawObject<T, StorageType>, StorageType>;
		using AsyncWaiterPtr = std::unique_ptr<AsyncW>;
		...
		ShmemRawConsumer(const std::string& t_producerName, const std::string& t_memoryName, const std::string& t_consumerName) 
		...
		*/
		...
		bool poll(PollCallback&& t_get)
		{
			...
			return sharedPtr->poll(m_consumer, std::forward<PollCallback>(t_get));
		}
		...
		*/
		bool pollCopy(reference_type t_dataOut)
		{
			checkObject();
			auto sharedPtr = SharedObjectSharedPtr(boost::move(referenceObject()));
			return sharedPtr->pollCopy(m_consumer, std::forward<reference_type>(t_dataOut));
		}
		...
		*/
		...
		bool waitCopy(reference_type t_dataOut, const std::chrono::duration<_Rep, _Period>& t_timeout)
		{
			checkObject();
			auto sharedPtr = SharedObjectSharedPtr(boost::move(referenceObject()));
			return sharedPtr->waitCopy(m_consumer, t_timeout, std::forward<reference_type>(t_dataOut));
		}
		...
		*/
		...
		bool wait(const std::chrono::duration<_Rep, _Period>& t_timeout, RecieveCallback&& t_get)
		{
			checkObject();
			auto sharedPtr = SharedObjectSharedPtr(boost::move(referenceObject()));
			return sharedPtr->wait(m_consumer, t_timeout, std::forward<RecieveCallback>(t_get));
		}
		...
		*/
		...
		bool asyncWait(RecieveCallback&& t_get)
		{
			...
			m_asyncWaiter.reset();
			auto sharedPtr = SharedObjectSharedPtr(boost::move(referenceObject()));
			m_asyncWaiter = std::make_unique<AsyncW>(sharedPtr, m_consumer, std::forward<RecieveCallback>(t_get));
	...
	private:
		...
		*/
		AsyncWaiterPtr m_asyncWaiter;
	};
}
#endif // !SHMEM_RAW_CONSUMER_HPP