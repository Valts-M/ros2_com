...
namespace ouster {
    namespace OS1 {
        ...
        std::function<void(const uint8_t*, iterator_type it)> batch_to_iter(
            const std::vector<double>& xyz_lut, int W, int H, const unsigned char altitude_index,
            ...
            return [=](const uint8_t* packet_buf, iterator_type it) mutable {
                for (int icol = 0; icol < OS1::columns_per_buffer; icol++) {
                    ...
                    /*const float enc = OS1::col_h_angle(col_buf);*/
                    /*const uint32_t enc_c = OS1::col_h_encoder_count(col_buf);
                    if (enc_c > 67584 || enc_c < 22528) continue;*/ // corresponds to 90 and 270 degrees in lidar frame
                    ...
                    #ifndef USE_ALTITUDE_INDEX
                    for (uint8_t ipx = 0; ipx < H; ipx++)
                        #else // USE_ALTITUDE_INDEX
                    int ipx = altitude_index;
                    #endif // !USE_ALTITUDE_INDEX
                    {
                        #ifdef REDUCE
                        const uint8_t* px_buf = OS1::nth_px(ipx * 4 + 2, col_buf);
                        #else // !REDUCE
                        const uint8_t* px_buf = OS1::nth_px(ipx, col_buf);
                        #endif // REDUCE
                        int ind = 5 * (idx + ipx);
                        double r = (double)OS1::px_range(px_buf);
            
                        // x, y, z(m), i, ts, reflectivity, ring, noise, range (mm)
                        it[idx + ipx] = c(r * 0.001 * xyz_lut[ind + 0],
                            r * 0.001 * xyz_lut[ind + 1],
                            r * 0.001 * xyz_lut[ind + 2],
