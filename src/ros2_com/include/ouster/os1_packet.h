...
#ifndef _OS1_PACKET_H_
#define _OS1_PACKET_H_
...
namespace ouster {
  ...
  inline float col_h_angle(const uint8_t * col_buf)
  {
    ...
    return (float)(2.0 * M_PI * (double)ticks / (double)encoder_ticks_per_rev);
  }

  inline uint32_t col_h_encoder_count(const uint8_t * col_buf)
  {
    uint32_t res;
    memcpy(&res, col_buf + 12, sizeof(uint32_t));
    return res;
  }
  ...
  inline uint32_t col_valid(const uint8_t * col_buf)
  {
    uint32_t res;
    memcpy(
      &res, col_buf + (16 + pixels_per_column * pixel_bytes),
      sizeof(uint32_t));
    return res;
  }
